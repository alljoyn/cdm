...
{
    if (!m_intfControllee) {
        CdmInterface* cdmInterface = m_sample->CreateInterface(REPEAT_MODE_INTERFACE, m_objectPath, m_listener);
        if (!cdmInterface) {
            cout << "Interface creation failed." << endl;
            return;
        }

        m_intfControllee = static_cast<RepeatModeIntfControllee*>(cdmInterface);
