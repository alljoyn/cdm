    ...
    public LanguageDisplayInterface
{
  ...
  public:
    static Impl* CreateInterface(BusAttachment& busAttachment, Ref<LanguageDisplayIntfControlleeModel> model, CdmBusObject& cdmBusObject, CdmControllee& cdmControllee);
  ...
  private:
     ...
     */
    Impl(BusAttachment& busAttachment, Ref<LanguageDisplayIntfControlleeModel> model, CdmBusObject& cdmBusObject, CdmControllee& cdmControllee);

    QStatus ValidateDisplayLanguage(qcc::String& value);
 ...
 * Forwarding
 */
LanguageDisplayIntfControllee::LanguageDisplayIntfControllee(BusAttachment& busAttachment, Ref<InterfaceControlleeModel> model, CdmBusObject& cdmBusObject, CdmControllee& cdmControllee)
    : m_impl(Impl::CreateInterface(busAttachment, dynamic_pointer_cast<LanguageDisplayIntfControlleeModel>(model), cdmBusObject, cdmControllee))
{}
...
{
    LanguageDisplayIntfControllee::Impl* interface = new LanguageDisplayIntfControllee::Impl(busAttachment, model, cdmBusObject, cdmControllee);
...
LanguageDisplayIntfControllee::Impl::Impl(BusAttachment& busAttachment, Ref<LanguageDisplayIntfControlleeModel> model, CdmBusObject& cdmBusObject, CdmControllee& cdmControllee) :
...
QStatus LanguageDisplayIntfControllee::Impl::OnSetProperty(const String& propName, MsgArg& msgarg)
{
    if (!(s_prop_DisplayLanguage.compare(propName))) {
        ...
        if (ValidateDisplayLanguage(validValue) != ER_OK)
            return ER_INVALID_DATA;

        QStatus status = m_LanguageDisplayModelInterface->SetDisplayLanguage(validValue);
        if (status != ER_OK) {
...
void LanguageDisplayIntfControllee::Impl::OnMethodHandler(const InterfaceDescription::Member* member, Message& msg)
{
    bool isFound = false;
...
QStatus LanguageDisplayIntfControllee::Impl::ValidateDisplayLanguage(qcc::String& value)
{

    std::vector<qcc::String> validValues;
    if (m_LanguageDisplayModelInterface->GetSupportedDisplayLanguages(validValues) != ER_OK)
        return ER_FAIL;
