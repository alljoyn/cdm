    ...
    public TimeDisplayInterface
{
  ...
  public:
    static Impl* CreateInterface(BusAttachment& busAttachment, Ref<TimeDisplayIntfControlleeModel> model, CdmBusObject& cdmBusObject);
  ...
  private:
     ...
     */
    Impl(BusAttachment& busAttachment, Ref<TimeDisplayIntfControlleeModel> model, CdmBusObject& cdmBusObject);
 ...
 * Forwarding
 */
TimeDisplayIntfControllee::TimeDisplayIntfControllee(BusAttachment& busAttachment, Ref<InterfaceControlleeModel> model, CdmBusObject& cdmBusObject)
    : m_impl(Impl::CreateInterface(busAttachment, dynamic_pointer_cast<TimeDisplayIntfControlleeModel>(model), cdmBusObject))
{}
...
{
    TimeDisplayIntfControllee::Impl* interface = new TimeDisplayIntfControllee::Impl(busAttachment, model, cdmBusObject);
...
TimeDisplayIntfControllee::Impl::Impl(BusAttachment& busAttachment, Ref<TimeDisplayIntfControlleeModel> model, CdmBusObject& cdmBusObject) :
...
QStatus TimeDisplayIntfControllee::Impl::OnSetProperty(const String& propName, MsgArg& msgarg)
{
    if (!(s_prop_DisplayTimeFormat.compare(propName))) {
        ...
        QStatus status;
        status = m_TimeDisplayModelInterface->SetDisplayTimeFormat(value);
        if (status != ER_OK) {
            QCC_LogError(status, ("%s: failed to set property value", __func__));
            return ER_BUS_PROPERTY_VALUE_NOT_SET;
        }

        EmitDisplayTimeFormatChanged(value);
...
void TimeDisplayIntfControllee::Impl::OnMethodHandler(const InterfaceDescription::Member* member, Message& msg, CdmControllee& controllee)
{
    bool isFound = false;
