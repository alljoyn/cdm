 ...
 */
class WaterLevelIntfControllee::Impl :
    public InterfaceReceiver,
    public WaterLevelInterface
{

  public:
    static Impl* CreateInterface(BusAttachment& busAttachment, Ref<WaterLevelIntfControlleeModel> model, CdmBusObject& cdmBusObject, CdmControllee& cdmControllee);
  ...
  private:
    /**
     * Constructor of WaterLevelIntfControllee::Impl
     */
    Impl(BusAttachment& busAttachment, Ref<WaterLevelIntfControlleeModel> model, CdmBusObject& cdmBusObject, CdmControllee& cdmControllee);
 ...
 * Forwarding
 */
WaterLevelIntfControllee::WaterLevelIntfControllee(BusAttachment& busAttachment, Ref<InterfaceControlleeModel> model, CdmBusObject& cdmBusObject, CdmControllee& cdmControllee)
    : m_impl(Impl::CreateInterface(busAttachment, dynamic_pointer_cast<WaterLevelIntfControlleeModel>(model), cdmBusObject, cdmControllee))
{}


WaterLevelIntfControllee::~WaterLevelIntfControllee()
{
    delete m_impl;
}

const qcc::String& WaterLevelIntfControllee::GetInterfaceName() const
{
    return m_impl->GetInterfaceName();
}

QStatus WaterLevelIntfControllee::EmitSupplySourceChanged(const WaterLevelInterface::SupplySource newValue)
{
    return (m_impl == nullptr) ? ER_FAIL : m_impl->EmitSupplySourceChanged(newValue);
}

QStatus WaterLevelIntfControllee::EmitCurrentLevelChanged(const uint8_t newValue)
{
    return (m_impl == nullptr) ? ER_FAIL : m_impl->EmitCurrentLevelChanged(newValue);
}

QStatus WaterLevelIntfControllee::EmitMaxLevelChanged(const uint8_t newValue)
{
 ...
 * Implementation
 */
 WaterLevelIntfControllee::Impl* WaterLevelIntfControllee::Impl::CreateInterface(
    BusAttachment& busAttachment,
    Ref<WaterLevelIntfControlleeModel> model,
    CdmBusObject& cdmBusObject,
    CdmControllee& cdmControllee)
{
    WaterLevelIntfControllee::Impl* interface = new WaterLevelIntfControllee::Impl(busAttachment, model, cdmBusObject, cdmControllee);
...
WaterLevelIntfControllee::Impl::Impl(BusAttachment& busAttachment, Ref<WaterLevelIntfControlleeModel> model, CdmBusObject& cdmBusObject, CdmControllee& cdmControllee) :
...
QStatus WaterLevelIntfControllee::Impl::Init()
{
...
QStatus WaterLevelIntfControllee::Impl::OnGetProperty(const String& propName, MsgArg& msgarg)
{
    if (!(s_prop_Version.compare(propName))) {
        ...
        if (!(s_prop_SupplySource.compare(propName))) {
            WaterLevelInterface::SupplySource value;
            auto status = m_WaterLevelModelInterface->GetSupplySource(value);
            if (status != ER_OK) {
            ...
            uint8_t value;
            auto status = m_WaterLevelModelInterface->GetCurrentLevel(value);
            if (status != ER_OK) {
                QCC_LogError(status, ("%s: failed to get actual property value from application. use previous value.", __func__));
                return ER_FAIL;
            }
            ...
            uint8_t value;
            auto status = m_WaterLevelModelInterface->GetMaxLevel(value);
            if (status != ER_OK) {
                QCC_LogError(status, ("%s: failed to get actual property value from application. use previous value.", __func__));
                return ER_FAIL;
            }
...
QStatus WaterLevelIntfControllee::Impl::OnSetProperty(const String& propName, MsgArg& msgarg)
{
    if (!(s_prop_SupplySource.compare(propName))) {
...
void WaterLevelIntfControllee::Impl::OnMethodHandler(const InterfaceDescription::Member* member, Message& msg)
{
    bool isFound = false;
...
QStatus WaterLevelIntfControllee::Impl::EmitSupplySourceChanged(const WaterLevelInterface::SupplySource newValue)
{
    ...
    m_busObject.EmitPropChanged(GetInterfaceName().c_str(), s_prop_SupplySource.c_str(), val, SESSION_ID_ALL_HOSTED, ALLJOYN_FLAG_GLOBAL_BROADCAST);

    return status;
}

QStatus WaterLevelIntfControllee::Impl::EmitCurrentLevelChanged(const uint8_t newValue)
{
    QStatus status = ER_OK;
...
QStatus WaterLevelIntfControllee::Impl::EmitMaxLevelChanged(const uint8_t newValue)
{
    QStatus status = ER_OK;
