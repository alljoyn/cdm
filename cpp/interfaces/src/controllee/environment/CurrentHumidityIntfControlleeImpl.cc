 ...
 */
class CurrentHumidityIntfControllee::Impl :
    public InterfaceReceiver,
    public CurrentHumidityInterface
{

  public:
    static Impl* CreateInterface(BusAttachment& busAttachment, Ref<CurrentHumidityIntfControlleeModel> model, CdmBusObject& cdmBusObject, CdmControllee& cdmControllee);
  ...
  private:
    /**
     * Constructor of CurrentHumidityIntfControllee::Impl
     */
    Impl(BusAttachment& busAttachment, Ref<CurrentHumidityIntfControlleeModel> model, CdmBusObject& cdmBusObject, CdmControllee& cdmControllee);

    BusAttachment& m_busAttachment;
    CdmControllee& m_cdmControllee;
    MethodHandlers m_methodHandlers;

    Ref<CurrentHumidityIntfControlleeModel> m_CurrentHumidityModelInterface;
 ...
 * Forwarding
 */
CurrentHumidityIntfControllee::CurrentHumidityIntfControllee(BusAttachment& busAttachment, Ref<InterfaceControlleeModel> model, CdmBusObject& cdmBusObject, CdmControllee& cdmControllee)
    : m_impl(Impl::CreateInterface(busAttachment, dynamic_pointer_cast<CurrentHumidityIntfControlleeModel>(model), cdmBusObject, cdmControllee))
{}


CurrentHumidityIntfControllee::~CurrentHumidityIntfControllee()
{
    delete m_impl;
}

const qcc::String& CurrentHumidityIntfControllee::GetInterfaceName() const
{
    return m_impl->GetInterfaceName();
}

QStatus CurrentHumidityIntfControllee::EmitCurrentValueChanged(const uint8_t newValue)
{
    return (m_impl == nullptr) ? ER_FAIL : m_impl->EmitCurrentValueChanged(newValue);
}

QStatus CurrentHumidityIntfControllee::EmitMaxValueChanged(const uint8_t newValue)
{
 ...
 * Implementation
 */
 CurrentHumidityIntfControllee::Impl* CurrentHumidityIntfControllee::Impl::CreateInterface(
    BusAttachment& busAttachment,
    Ref<CurrentHumidityIntfControlleeModel> model,
    CdmBusObject& cdmBusObject,
    CdmControllee& cdmControllee)
{
    CurrentHumidityIntfControllee::Impl* interface = new CurrentHumidityIntfControllee::Impl(busAttachment, model, cdmBusObject, cdmControllee);
...
CurrentHumidityIntfControllee::Impl::Impl(BusAttachment& busAttachment, Ref<CurrentHumidityIntfControlleeModel> model, CdmBusObject& cdmBusObject, CdmControllee& cdmControllee) :
    InterfaceReceiver(cdmBusObject),
    m_busAttachment(busAttachment),
    m_cdmControllee(cdmControllee),
    m_methodHandlers(),
    m_CurrentHumidityModelInterface(model),
    InterfaceName(CurrentHumidityInterface::INTERFACE_NAME)
{
    ignore_unused(m_cdmControllee);
}


QStatus CurrentHumidityIntfControllee::Impl::Init()
{
...
QStatus CurrentHumidityIntfControllee::Impl::OnGetProperty(const String& propName, MsgArg& msgarg)
{
    if (!(s_prop_Version.compare(propName))) {
        ...
        if (!(s_prop_CurrentValue.compare(propName))) {
            uint8_t value;
            auto status = m_CurrentHumidityModelInterface->GetCurrentValue(value);
            if (status != ER_OK) {
            ...
            uint8_t value;
            auto status = m_CurrentHumidityModelInterface->GetMaxValue(value);
            if (status != ER_OK) {
                QCC_LogError(status, ("%s: failed to get actual property value from application. use previous value.", __func__));
                return ER_FAIL;
            }
...
QStatus CurrentHumidityIntfControllee::Impl::OnSetProperty(const String& propName, MsgArg& msgarg)
{
    if (!(s_prop_CurrentValue.compare(propName))) {
...
void CurrentHumidityIntfControllee::Impl::OnMethodHandler(const InterfaceDescription::Member* member, Message& msg)
{
    bool isFound = false;
...
QStatus CurrentHumidityIntfControllee::Impl::EmitCurrentValueChanged(const uint8_t newValue)
{
...
QStatus CurrentHumidityIntfControllee::Impl::EmitMaxValueChanged(const uint8_t newValue)
{
    QStatus status = ER_OK;
