    ...
    public WindDirectionInterface
{
  ...
  public:
    static Impl* CreateInterface(BusAttachment& busAttachment, Ref<WindDirectionIntfControlleeModel> model, CdmBusObject& cdmBusObject, CdmControllee& cdmControllee);
  ...
  private:
     ...
     */
    Impl(BusAttachment& busAttachment, Ref<WindDirectionIntfControlleeModel> model, CdmBusObject& cdmBusObject, CdmControllee& cdmControllee);

    QStatus clampHorizontalDirection(uint16_t value, uint16_t& out);

    QStatus ValidateHorizontalAutoMode(WindDirectionInterface::AutoMode value);

    QStatus clampVerticalDirection(uint16_t value, uint16_t& out);

    QStatus ValidateVerticalAutoMode(WindDirectionInterface::AutoMode value);
 ...
 * Forwarding
 */
WindDirectionIntfControllee::WindDirectionIntfControllee(BusAttachment& busAttachment, Ref<InterfaceControlleeModel> model, CdmBusObject& cdmBusObject, CdmControllee& cdmControllee)
    : m_impl(Impl::CreateInterface(busAttachment, dynamic_pointer_cast<WindDirectionIntfControlleeModel>(model), cdmBusObject, cdmControllee))
{}
...
{
    WindDirectionIntfControllee::Impl* interface = new WindDirectionIntfControllee::Impl(busAttachment, model, cdmBusObject, cdmControllee);
...
WindDirectionIntfControllee::Impl::Impl(BusAttachment& busAttachment, Ref<WindDirectionIntfControlleeModel> model, CdmBusObject& cdmBusObject, CdmControllee& cdmControllee) :
    InterfaceReceiver(cdmBusObject),
    m_busAttachment(busAttachment),
    m_cdmControllee(cdmControllee),
    m_methodHandlers(),
    m_WindDirectionModelInterface(model),
    InterfaceName(WindDirectionInterface::INTERFACE_NAME)
{
...
QStatus WindDirectionIntfControllee::Impl::OnSetProperty(const String& propName, MsgArg& msgarg)
{
    if (!(s_prop_HorizontalDirection.compare(propName))) {
        ...
        if (clampHorizontalDirection(value, validValue) != ER_OK)
            return ER_INVALID_DATA;

        QStatus status = m_WindDirectionModelInterface->SetHorizontalDirection(validValue);
        if (status != ER_OK) {
        ...
        WindDirectionInterface::AutoMode validValue = value;

        if (ValidateHorizontalAutoMode(validValue) != ER_OK)
            return ER_INVALID_DATA;

        QStatus status = m_WindDirectionModelInterface->SetHorizontalAutoMode(validValue);
        if (status != ER_OK) {
            QCC_LogError(status, ("%s: failed to set property value", __func__));
            return ER_BUS_PROPERTY_VALUE_NOT_SET;
        }
        ...
        if (clampVerticalDirection(value, validValue) != ER_OK)
            return ER_INVALID_DATA;

        QStatus status = m_WindDirectionModelInterface->SetVerticalDirection(validValue);
        if (status != ER_OK) {
            QCC_LogError(status, ("%s: failed to set property value", __func__));
            return ER_BUS_PROPERTY_VALUE_NOT_SET;
        }
        ...
        if (ValidateVerticalAutoMode(validValue) != ER_OK)
            return ER_INVALID_DATA;

        QStatus status = m_WindDirectionModelInterface->SetVerticalAutoMode(validValue);
        if (status != ER_OK) {
            QCC_LogError(status, ("%s: failed to set property value", __func__));
            return ER_BUS_PROPERTY_VALUE_NOT_SET;
        }
...
void WindDirectionIntfControllee::Impl::OnMethodHandler(const InterfaceDescription::Member* member, Message& msg)
{
    bool isFound = false;
...
QStatus WindDirectionIntfControllee::Impl::clampHorizontalDirection(uint16_t value, uint16_t& out)
{

    uint16_t minValue = 0;

    uint16_t maxValue;
    if (m_WindDirectionModelInterface->GetHorizontalMax(maxValue) != ER_OK)
        return ER_FAIL;
...
QStatus WindDirectionIntfControllee::Impl::ValidateHorizontalAutoMode(WindDirectionInterface::AutoMode value)
{
    switch (value)
    {
        ...
        case WindDirectionInterface::AUTO_MODE_ON:
            break;
        case WindDirectionInterface::AUTO_MODE_NOT_SUPPORTED:
        default:
            return ER_INVALID_DATA;
    }
    return ER_OK;
...
QStatus WindDirectionIntfControllee::Impl::clampVerticalDirection(uint16_t value, uint16_t& out)
{

    uint16_t minValue = 0;

    uint16_t maxValue;
    if (m_WindDirectionModelInterface->GetVerticalMax(maxValue) != ER_OK)
        return ER_FAIL;
...
QStatus WindDirectionIntfControllee::Impl::ValidateVerticalAutoMode(WindDirectionInterface::AutoMode value)
{
    switch (value)
    {
        case WindDirectionInterface::AUTO_MODE_OFF:
