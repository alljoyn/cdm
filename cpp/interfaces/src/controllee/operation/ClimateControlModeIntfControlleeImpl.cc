...
QStatus ClimateControlModeIntfControllee::EmitModeChanged(const Mode newValue)
{
    return (m_impl == nullptr) ? ER_FAIL : m_impl->EmitModeChanged(newValue);
}

QStatus ClimateControlModeIntfControllee::EmitSupportedModesChanged(const std::vector<Mode> newValue)
{
    return (m_impl == nullptr) ? ER_FAIL : m_impl->EmitSupportedModesChanged(newValue);
}

QStatus ClimateControlModeIntfControllee::EmitOperationalStateChanged(const OperationalState newValue)
{
...
QStatus ClimateControlModeIntfControllee::Impl::OnGetProperty(const String& propName, MsgArg& msgarg)
{
    ...
    if (!(s_prop_Version.compare(propName))) {
        ...
        if (!(s_prop_Mode.compare(propName))) {
            Mode value;
            status = m_ClimateControlModeModelInterface->GetMode(value);
            if (status != ER_OK) {
            ...
            status = m_ClimateControlModeModelInterface->GetSupportedModes(value);
            if (status != ER_OK) {
                QCC_LogError(status, ("%s: failed to get actual property value from application. use previous value.", __func__));
                return ER_FAIL;
            }
            ...
            status = m_ClimateControlModeModelInterface->GetOperationalState(value);
            if (status != ER_OK) {
                QCC_LogError(status, ("%s: failed to get actual property value from application. use previous value.", __func__));
                return ER_FAIL;
            }
...
void ClimateControlModeIntfControllee::Impl::OnMethodHandler(const InterfaceDescription::Member* member, Message& msg, CdmSideEffects& sideEffects)
{
...
QStatus ClimateControlModeIntfControllee::Impl::EmitModeChanged(const Mode newValue)
{
    QStatus status = ER_OK;
...
QStatus ClimateControlModeIntfControllee::Impl::EmitSupportedModesChanged(const std::vector<Mode> newValue)
{
    QStatus status = ER_OK;
...
QStatus ClimateControlModeIntfControllee::Impl::EmitOperationalStateChanged(const OperationalState newValue)
{
    QStatus status = ER_OK;
