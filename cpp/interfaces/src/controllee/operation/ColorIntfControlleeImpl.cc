    ...
    public ColorInterface
{
  ...
  public:
    static Impl* CreateInterface(BusAttachment& busAttachment, Ref<ColorIntfControlleeModel> model, CdmBusObject& cdmBusObject, CdmControllee& cdmControllee);
  ...
  private:
     ...
     */
    Impl(BusAttachment& busAttachment, Ref<ColorIntfControlleeModel> model, CdmBusObject& cdmBusObject, CdmControllee& cdmControllee);

    QStatus clampHue(double value, double& out);

    QStatus clampSaturation(double value, double& out);
 ...
 * Forwarding
 */
ColorIntfControllee::ColorIntfControllee(BusAttachment& busAttachment, Ref<InterfaceControlleeModel> model, CdmBusObject& cdmBusObject, CdmControllee& cdmControllee)
    : m_impl(Impl::CreateInterface(busAttachment, dynamic_pointer_cast<ColorIntfControlleeModel>(model), cdmBusObject, cdmControllee))
{}
...
{
    ColorIntfControllee::Impl* interface = new ColorIntfControllee::Impl(busAttachment, model, cdmBusObject, cdmControllee);
...
ColorIntfControllee::Impl::Impl(BusAttachment& busAttachment, Ref<ColorIntfControlleeModel> model, CdmBusObject& cdmBusObject, CdmControllee& cdmControllee) :
    InterfaceReceiver(cdmBusObject),
    m_busAttachment(busAttachment),
    m_cdmControllee(cdmControllee),
    m_methodHandlers(),
    m_ColorModelInterface(model),
    InterfaceName(ColorInterface::INTERFACE_NAME)
{
...
QStatus ColorIntfControllee::Impl::OnSetProperty(const String& propName, MsgArg& msgarg)
{
    if (!(s_prop_Hue.compare(propName))) {
        ...
        if (clampHue(value, validValue) != ER_OK)
            return ER_INVALID_DATA;

        QStatus status = m_ColorModelInterface->SetHue(validValue);
        if (status != ER_OK) {
        ...
        if (clampSaturation(value, validValue) != ER_OK)
            return ER_INVALID_DATA;

        QStatus status = m_ColorModelInterface->SetSaturation(validValue);
        if (status != ER_OK) {
            QCC_LogError(status, ("%s: failed to set property value", __func__));
            return ER_BUS_PROPERTY_VALUE_NOT_SET;
        }
...
void ColorIntfControllee::Impl::OnMethodHandler(const InterfaceDescription::Member* member, Message& msg)
{
    bool isFound = false;
...
QStatus ColorIntfControllee::Impl::clampHue(double value, double& out)
{
...
QStatus ColorIntfControllee::Impl::clampSaturation(double value, double& out)
{
