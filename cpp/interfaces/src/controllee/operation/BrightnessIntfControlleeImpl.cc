...
QStatus BrightnessIntfControllee::Impl::OnGetProperty(const String& propName, MsgArg& msgarg)
{
    ...
    if (!(s_prop_Version.compare(propName))) {
        ...
        if (!(s_prop_Brightness.compare(propName))) {
            double value;
            status = m_BrightnessModelInterface->GetBrightness(value);
            if (status != ER_OK) {
...
void BrightnessIntfControllee::Impl::OnMethodHandler(const InterfaceDescription::Member* member, Message& msg, CdmSideEffects& sideEffects)
{
