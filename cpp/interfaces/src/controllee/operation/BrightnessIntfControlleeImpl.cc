    ...
    public BrightnessInterface
{
  ...
  public:
    static Impl* CreateInterface(BusAttachment& busAttachment, Ref<BrightnessIntfControlleeModel> model, CdmBusObject& cdmBusObject, CdmControllee& cdmControllee);
  ...
  private:
     ...
     */
    Impl(BusAttachment& busAttachment, Ref<BrightnessIntfControlleeModel> model, CdmBusObject& cdmBusObject, CdmControllee& cdmControllee);

    QStatus clampBrightness(double value, double& out);
 ...
 * Forwarding
 */
BrightnessIntfControllee::BrightnessIntfControllee(BusAttachment& busAttachment, Ref<InterfaceControlleeModel> model, CdmBusObject& cdmBusObject, CdmControllee& cdmControllee)
    : m_impl(Impl::CreateInterface(busAttachment, dynamic_pointer_cast<BrightnessIntfControlleeModel>(model), cdmBusObject, cdmControllee))
{}
...
{
    BrightnessIntfControllee::Impl* interface = new BrightnessIntfControllee::Impl(busAttachment, model, cdmBusObject, cdmControllee);
...
BrightnessIntfControllee::Impl::Impl(BusAttachment& busAttachment, Ref<BrightnessIntfControlleeModel> model, CdmBusObject& cdmBusObject, CdmControllee& cdmControllee) :
...
QStatus BrightnessIntfControllee::Impl::OnSetProperty(const String& propName, MsgArg& msgarg)
{
    if (!(s_prop_Brightness.compare(propName))) {
        ...
        if (clampBrightness(value, validValue) != ER_OK)
            return ER_INVALID_DATA;

        QStatus status = m_BrightnessModelInterface->SetBrightness(validValue);
        if (status != ER_OK) {
...
void BrightnessIntfControllee::Impl::OnMethodHandler(const InterfaceDescription::Member* member, Message& msg)
{
    bool isFound = false;
...
QStatus BrightnessIntfControllee::Impl::clampBrightness(double value, double& out)
{
