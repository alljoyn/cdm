    ...
    public AudioVolumeInterface
{
  ...
  public:
    static Impl* CreateInterface(BusAttachment& busAttachment, Ref<AudioVolumeIntfControlleeModel> model, CdmBusObject& cdmBusObject, CdmControllee& cdmControllee);
  ...
  private:
     ...
     */
    Impl(BusAttachment& busAttachment, Ref<AudioVolumeIntfControlleeModel> model, CdmBusObject& cdmBusObject, CdmControllee& cdmControllee);

    QStatus clampVolume(uint8_t value, uint8_t& out);
 ...
 * Forwarding
 */
AudioVolumeIntfControllee::AudioVolumeIntfControllee(BusAttachment& busAttachment, Ref<InterfaceControlleeModel> model, CdmBusObject& cdmBusObject, CdmControllee& cdmControllee)
    : m_impl(Impl::CreateInterface(busAttachment, dynamic_pointer_cast<AudioVolumeIntfControlleeModel>(model), cdmBusObject, cdmControllee))
{}
...
{
    AudioVolumeIntfControllee::Impl* interface = new AudioVolumeIntfControllee::Impl(busAttachment, model, cdmBusObject, cdmControllee);
...
AudioVolumeIntfControllee::Impl::Impl(BusAttachment& busAttachment, Ref<AudioVolumeIntfControlleeModel> model, CdmBusObject& cdmBusObject, CdmControllee& cdmControllee) :
...
QStatus AudioVolumeIntfControllee::Impl::OnGetProperty(const String& propName, MsgArg& msgarg)
{
    if (!(s_prop_Version.compare(propName))) {
        ...
        if (!(s_prop_Volume.compare(propName))) {
            uint8_t value;
            auto status = m_AudioVolumeModelInterface->GetVolume(value);
            if (status != ER_OK) {
            ...
            uint8_t value;
            auto status = m_AudioVolumeModelInterface->GetMaxVolume(value);
            if (status != ER_OK) {
                QCC_LogError(status, ("%s: failed to get actual property value from application. use previous value.", __func__));
                return ER_FAIL;
            }
            ...
            bool value;
            auto status = m_AudioVolumeModelInterface->GetMute(value);
            if (status != ER_OK) {
                QCC_LogError(status, ("%s: failed to get actual property value from application. use previous value.", __func__));
                return ER_FAIL;
            }
...
QStatus AudioVolumeIntfControllee::Impl::OnSetProperty(const String& propName, MsgArg& msgarg)
{
    if (!(s_prop_Volume.compare(propName))) {
        ...
        if (clampVolume(value, validValue) != ER_OK)
            return ER_INVALID_DATA;

        QStatus status = m_AudioVolumeModelInterface->SetVolume(validValue);
        if (status != ER_OK) {
        ...
        QStatus status = m_AudioVolumeModelInterface->SetMute(validValue);
        if (status != ER_OK) {
            QCC_LogError(status, ("%s: failed to set property value", __func__));
            return ER_BUS_PROPERTY_VALUE_NOT_SET;
        }
...
void AudioVolumeIntfControllee::Impl::OnMethodHandler(const InterfaceDescription::Member* member, Message& msg)
{
    bool isFound = false;
...
QStatus AudioVolumeIntfControllee::Impl::clampVolume(uint8_t value, uint8_t& out)
{

    uint8_t minValue = 0;

    uint8_t maxValue;
    if (m_AudioVolumeModelInterface->GetMaxVolume(maxValue) != ER_OK)
        return ER_FAIL;
