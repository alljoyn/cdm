    ...
    public TimerInterface
{
  ...
  public:
     ...
     */
    void OnSetTargetTimeToStart(const InterfaceDescription::Member* member, Message& msg, CdmSideEffects& sideEffects);
     ...
     * @param msg reference of alljoyn Message
     */
    void OnSetTargetTimeToStop(const InterfaceDescription::Member* member, Message& msg, CdmSideEffects& sideEffects);
...
QStatus TimerIntfControllee::Impl::OnGetProperty(const String& propName, MsgArg& msgarg)
{
    ...
    if (!(s_prop_Version.compare(propName))) {
        ...
        if (!(s_prop_ReferenceTimer.compare(propName))) {
            int32_t value;
            status = m_TimerModelInterface->GetReferenceTimer(value);
            if (status != ER_OK) {
            ...
            int32_t value;
            status = m_TimerModelInterface->GetTargetTimeToStart(value);
            if (status != ER_OK) {
                QCC_LogError(status, ("%s: failed to get actual property value from application. use previous value.", __func__));
                return ER_FAIL;
            }
            ...
            int32_t value;
            status = m_TimerModelInterface->GetTargetTimeToStop(value);
            if (status != ER_OK) {
                QCC_LogError(status, ("%s: failed to get actual property value from application. use previous value.", __func__));
                return ER_FAIL;
            }
            ...
            int32_t value;
            status = m_TimerModelInterface->GetEstimatedTimeToEnd(value);
            if (status != ER_OK) {
                QCC_LogError(status, ("%s: failed to get actual property value from application. use previous value.", __func__));
                return ER_FAIL;
            }
            ...
            int32_t value;
            status = m_TimerModelInterface->GetRunningTime(value);
            if (status != ER_OK) {
                QCC_LogError(status, ("%s: failed to get actual property value from application. use previous value.", __func__));
                return ER_FAIL;
            }
            ...
            int32_t value;
            status = m_TimerModelInterface->GetTargetDuration(value);
            if (status != ER_OK) {
                QCC_LogError(status, ("%s: failed to get actual property value from application. use previous value.", __func__));
                return ER_FAIL;
            }
...
void TimerIntfControllee::Impl::OnMethodHandler(const InterfaceDescription::Member* member, Message& msg, CdmSideEffects& sideEffects)
{
...
void TimerIntfControllee::Impl::OnSetTargetTimeToStart(const InterfaceDescription::Member* member, Message& msg, CdmSideEffects& sideEffects) {
    QStatus status = ER_OK;
    ...
    status = m_TimerModelInterface->SetTargetTimeToStart(arg_targetTimeToStart, errorCode, sideEffects);
...
void TimerIntfControllee::Impl::OnSetTargetTimeToStop(const InterfaceDescription::Member* member, Message& msg, CdmSideEffects& sideEffects) {
    QStatus status = ER_OK;
    const MsgArg* args = NULL;
    size_t numArgs = 0;
    ...
    ErrorCode errorCode = NOT_ERROR;
    status = m_TimerModelInterface->SetTargetTimeToStop(arg_targetTimeToStop, errorCode, sideEffects);
