    ...
    public DishWashingCyclePhaseInterface
{
  ...
  public:
    static Impl* CreateInterface(BusAttachment& busAttachment, Ref<DishWashingCyclePhaseIntfControlleeModel> model, CdmBusObject& cdmBusObject, CdmControllee& cdmControllee);
  ...
  private:
     ...
     */
    Impl(BusAttachment& busAttachment, Ref<DishWashingCyclePhaseIntfControlleeModel> model, CdmBusObject& cdmBusObject, CdmControllee& cdmControllee);
 ...
 * Forwarding
 */
DishWashingCyclePhaseIntfControllee::DishWashingCyclePhaseIntfControllee(BusAttachment& busAttachment, Ref<InterfaceControlleeModel> model, CdmBusObject& cdmBusObject, CdmControllee& cdmControllee)
    : m_impl(Impl::CreateInterface(busAttachment, dynamic_pointer_cast<DishWashingCyclePhaseIntfControlleeModel>(model), cdmBusObject, cdmControllee))
{}
...
{
    DishWashingCyclePhaseIntfControllee::Impl* interface = new DishWashingCyclePhaseIntfControllee::Impl(busAttachment, model, cdmBusObject, cdmControllee);
...
DishWashingCyclePhaseIntfControllee::Impl::Impl(BusAttachment& busAttachment, Ref<DishWashingCyclePhaseIntfControlleeModel> model, CdmBusObject& cdmBusObject, CdmControllee& cdmControllee) :
    InterfaceReceiver(cdmBusObject),
    m_busAttachment(busAttachment),
    m_cdmControllee(cdmControllee),
    m_methodHandlers(),
    m_DishWashingCyclePhaseModelInterface(model),
    InterfaceName(DishWashingCyclePhaseInterface::INTERFACE_NAME)
{
...
QStatus DishWashingCyclePhaseIntfControllee::Impl::OnGetProperty(const String& propName, MsgArg& msgarg)
{
    if (!(s_prop_Version.compare(propName))) {
        ...
        if (!(s_prop_CyclePhase.compare(propName))) {
            uint8_t value;
            auto status = m_DishWashingCyclePhaseModelInterface->GetCyclePhase(value);
            if (status != ER_OK) {
            ...
            auto status = m_DishWashingCyclePhaseModelInterface->GetSupportedCyclePhases(value);
            if (status != ER_OK) {
                QCC_LogError(status, ("%s: failed to get actual property value from application. use previous value.", __func__));
                return ER_FAIL;
            }
...
void DishWashingCyclePhaseIntfControllee::Impl::OnMethodHandler(const InterfaceDescription::Member* member, Message& msg)
{
    bool isFound = false;
...
void DishWashingCyclePhaseIntfControllee::Impl::OnGetVendorPhasesDescription(const InterfaceDescription::Member* member, Message& msg) {
    ...
    auto status = m_DishWashingCyclePhaseModelInterface->GetVendorPhasesDescription(arg_languageTag, arg_phasesDescription, errorCode, m_cdmControllee);

    // TODO this only allows one output arg
    MsgArg reply;
    CdmMsgCvt<std::vector<DishWashingCyclePhaseInterface::CyclePhaseDescriptor>> replyCvt;
