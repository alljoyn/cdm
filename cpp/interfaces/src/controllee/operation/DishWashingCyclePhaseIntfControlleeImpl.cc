 ...
 */
class DishWashingCyclePhaseIntfControllee::Impl :
    public InterfaceReceiver,
    public DishWashingCyclePhaseInterface
{

  public:
    static Impl* CreateInterface(BusAttachment& busAttachment, Ref<DishWashingCyclePhaseIntfControlleeModel> model, CdmBusObject& cdmBusObject);
  ...
  private:
    /**
     * Constructor of DishWashingCyclePhaseIntfControllee::Impl
     */
    Impl(BusAttachment& busAttachment, Ref<DishWashingCyclePhaseIntfControlleeModel> model, CdmBusObject& cdmBusObject);


    BusAttachment& m_busAttachment;
    Ref<DishWashingCyclePhaseIntfControlleeModel> m_DishWashingCyclePhaseModelInterface;
 ...
 * Forwarding
 */
DishWashingCyclePhaseIntfControllee::DishWashingCyclePhaseIntfControllee(BusAttachment& busAttachment, Ref<InterfaceControlleeModel> model, CdmBusObject& cdmBusObject)
    : m_impl(Impl::CreateInterface(busAttachment, dynamic_pointer_cast<DishWashingCyclePhaseIntfControlleeModel>(model), cdmBusObject))
{}


DishWashingCyclePhaseIntfControllee::~DishWashingCyclePhaseIntfControllee()
{
    delete m_impl;
}

const qcc::String& DishWashingCyclePhaseIntfControllee::GetInterfaceName() const
{
    return m_impl->GetInterfaceName();
}

QStatus DishWashingCyclePhaseIntfControllee::EmitCyclePhaseChanged(const uint8_t newValue)
{
    return (m_impl == nullptr) ? ER_FAIL : m_impl->EmitCyclePhaseChanged(newValue);
}

QStatus DishWashingCyclePhaseIntfControllee::EmitSupportedCyclePhasesChanged(const std::vector<uint8_t> newValue)
{
 ...
 * Implementation
 */
 DishWashingCyclePhaseIntfControllee::Impl* DishWashingCyclePhaseIntfControllee::Impl::CreateInterface(
    BusAttachment& busAttachment,
    Ref<DishWashingCyclePhaseIntfControlleeModel> model,
    CdmBusObject& cdmBusObject)
{
    DishWashingCyclePhaseIntfControllee::Impl* interface = new DishWashingCyclePhaseIntfControllee::Impl(busAttachment, model, cdmBusObject);
...
DishWashingCyclePhaseIntfControllee::Impl::Impl(BusAttachment& busAttachment, Ref<DishWashingCyclePhaseIntfControlleeModel> model, CdmBusObject& cdmBusObject) :
    InterfaceReceiver(cdmBusObject),
    m_busAttachment(busAttachment),
    m_DishWashingCyclePhaseModelInterface(model),
    m_methodHandlers()
{}


QStatus DishWashingCyclePhaseIntfControllee::Impl::Init()
{
...
QStatus DishWashingCyclePhaseIntfControllee::Impl::OnGetProperty(const String& propName, MsgArg& msgarg)
{
    QStatus status;
    ...
    if (!(s_prop_Version.compare(propName))) {
        ...
        if (!(s_prop_CyclePhase.compare(propName))) {
            uint8_t value;
            status = m_DishWashingCyclePhaseModelInterface->GetCyclePhase(value);
            if (status != ER_OK) {
            ...
            status = m_DishWashingCyclePhaseModelInterface->GetSupportedCyclePhases(value);
            if (status != ER_OK) {
                QCC_LogError(status, ("%s: failed to get actual property value from application. use previous value.", __func__));
                return ER_FAIL;
            }
...
QStatus DishWashingCyclePhaseIntfControllee::Impl::OnSetProperty(const String& propName, MsgArg& msgarg)
{
    QStatus status;
...
void DishWashingCyclePhaseIntfControllee::Impl::OnMethodHandler(const InterfaceDescription::Member* member, Message& msg, CdmSideEffects& sideEffects)
{
...
QStatus DishWashingCyclePhaseIntfControllee::Impl::EmitCyclePhaseChanged(const uint8_t newValue)
{
    QStatus status = ER_OK;
...
QStatus DishWashingCyclePhaseIntfControllee::Impl::EmitSupportedCyclePhasesChanged(const std::vector<uint8_t> newValue)
{
    QStatus status = ER_OK;
...
void DishWashingCyclePhaseIntfControllee::Impl::OnGetVendorPhasesDescription(const InterfaceDescription::Member* member, Message& msg, CdmSideEffects& sideEffects) {
    QStatus status = ER_OK;
    ...
    status = m_DishWashingCyclePhaseModelInterface->GetVendorPhasesDescription(arg_languageTag, arg_phasesDescription, errorCode, sideEffects);
