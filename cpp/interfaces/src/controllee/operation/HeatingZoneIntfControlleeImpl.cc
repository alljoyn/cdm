...
QStatus HeatingZoneIntfControllee::Impl::OnGetProperty(const String& propName, MsgArg& msgarg)
{
    ...
    if (!(s_prop_Version.compare(propName))) {
        ...
        if (!(s_prop_NumberOfHeatingZones.compare(propName))) {
            uint8_t value;
            status = m_HeatingZoneModelInterface->GetNumberOfHeatingZones(value);
            if (status != ER_OK) {
            ...
            status = m_HeatingZoneModelInterface->GetMaxHeatingLevels(value);
            if (status != ER_OK) {
                QCC_LogError(status, ("%s: failed to get actual property value from application. use previous value.", __func__));
                return ER_FAIL;
            }
            ...
            std::vector<uint8_t> value;
            status = m_HeatingZoneModelInterface->GetHeatingLevels(value);
            if (status != ER_OK) {
                QCC_LogError(status, ("%s: failed to get actual property value from application. use previous value.", __func__));
                return ER_FAIL;
            }
...
void HeatingZoneIntfControllee::Impl::OnMethodHandler(const InterfaceDescription::Member* member, Message& msg, CdmSideEffects& sideEffects)
{
