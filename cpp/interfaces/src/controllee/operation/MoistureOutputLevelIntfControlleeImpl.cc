    ...
    public MoistureOutputLevelInterface
{
  ...
  public:
    static Impl* CreateInterface(BusAttachment& busAttachment, Ref<MoistureOutputLevelIntfControlleeModel> model, CdmBusObject& cdmBusObject, CdmControllee& cdmControllee);
  ...
  private:
     ...
     */
    Impl(BusAttachment& busAttachment, Ref<MoistureOutputLevelIntfControlleeModel> model, CdmBusObject& cdmBusObject, CdmControllee& cdmControllee);

    QStatus clampMoistureOutputLevel(uint8_t value, uint8_t& out);

    QStatus ValidateAutoMode(MoistureOutputLevelInterface::AutoMode value);
 ...
 * Forwarding
 */
MoistureOutputLevelIntfControllee::MoistureOutputLevelIntfControllee(BusAttachment& busAttachment, Ref<InterfaceControlleeModel> model, CdmBusObject& cdmBusObject, CdmControllee& cdmControllee)
    : m_impl(Impl::CreateInterface(busAttachment, dynamic_pointer_cast<MoistureOutputLevelIntfControlleeModel>(model), cdmBusObject, cdmControllee))
{}
...
{
    MoistureOutputLevelIntfControllee::Impl* interface = new MoistureOutputLevelIntfControllee::Impl(busAttachment, model, cdmBusObject, cdmControllee);
...
MoistureOutputLevelIntfControllee::Impl::Impl(BusAttachment& busAttachment, Ref<MoistureOutputLevelIntfControlleeModel> model, CdmBusObject& cdmBusObject, CdmControllee& cdmControllee) :
...
QStatus MoistureOutputLevelIntfControllee::Impl::OnSetProperty(const String& propName, MsgArg& msgarg)
{
    if (!(s_prop_MoistureOutputLevel.compare(propName))) {
        ...
        if (clampMoistureOutputLevel(value, validValue) != ER_OK)
            return ER_INVALID_DATA;

        QStatus status = m_MoistureOutputLevelModelInterface->SetMoistureOutputLevel(validValue);
        if (status != ER_OK) {
        ...
        MoistureOutputLevelInterface::AutoMode validValue = value;

        if (ValidateAutoMode(validValue) != ER_OK)
            return ER_INVALID_DATA;

        QStatus status = m_MoistureOutputLevelModelInterface->SetAutoMode(validValue);
        if (status != ER_OK) {
            QCC_LogError(status, ("%s: failed to set property value", __func__));
            return ER_BUS_PROPERTY_VALUE_NOT_SET;
        }
...
void MoistureOutputLevelIntfControllee::Impl::OnMethodHandler(const InterfaceDescription::Member* member, Message& msg)
{
    bool isFound = false;
...
QStatus MoistureOutputLevelIntfControllee::Impl::clampMoistureOutputLevel(uint8_t value, uint8_t& out)
{

    uint8_t minValue = 0;

    uint8_t maxValue;
    if (m_MoistureOutputLevelModelInterface->GetMaxMoistureOutputLevel(maxValue) != ER_OK)
        return ER_FAIL;
...
QStatus MoistureOutputLevelIntfControllee::Impl::ValidateAutoMode(MoistureOutputLevelInterface::AutoMode value)
{
    switch (value)
    {
        ...
        case MoistureOutputLevelInterface::AUTO_MODE_ON:
            break;
        case MoistureOutputLevelInterface::AUTO_MODE_NOT_SUPPORTED:
        default:
            return ER_INVALID_DATA;
    }
    return ER_OK;
