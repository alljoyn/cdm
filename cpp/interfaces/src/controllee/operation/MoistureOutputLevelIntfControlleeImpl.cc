...
QStatus MoistureOutputLevelIntfControllee::EmitAutoModeChanged(const AutoMode newValue)
{
...
QStatus MoistureOutputLevelIntfControllee::Impl::OnGetProperty(const String& propName, MsgArg& msgarg)
{
    ...
    if (!(s_prop_Version.compare(propName))) {
        ...
        if (!(s_prop_MoistureOutputLevel.compare(propName))) {
            uint8_t value;
            status = m_MoistureOutputLevelModelInterface->GetMoistureOutputLevel(value);
            if (status != ER_OK) {
            ...
            uint8_t value;
            status = m_MoistureOutputLevelModelInterface->GetMaxMoistureOutputLevel(value);
            if (status != ER_OK) {
                QCC_LogError(status, ("%s: failed to get actual property value from application. use previous value.", __func__));
                return ER_FAIL;
            }
            ...
            AutoMode value;
            status = m_MoistureOutputLevelModelInterface->GetAutoMode(value);
            if (status != ER_OK) {
                QCC_LogError(status, ("%s: failed to get actual property value from application. use previous value.", __func__));
                return ER_FAIL;
            }
...
void MoistureOutputLevelIntfControllee::Impl::OnMethodHandler(const InterfaceDescription::Member* member, Message& msg, CdmSideEffects& sideEffects)
{
...
QStatus MoistureOutputLevelIntfControllee::Impl::EmitAutoModeChanged(const AutoMode newValue)
{
    QStatus status = ER_OK;
