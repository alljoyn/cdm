    ...
    public ColorTemperatureInterface
{
  ...
  public:
    static Impl* CreateInterface(BusAttachment& busAttachment, Ref<ColorTemperatureIntfControlleeModel> model, CdmBusObject& cdmBusObject, CdmControllee& cdmControllee);
  ...
  private:
     ...
     */
    Impl(BusAttachment& busAttachment, Ref<ColorTemperatureIntfControlleeModel> model, CdmBusObject& cdmBusObject, CdmControllee& cdmControllee);

    QStatus clampTemperature(double value, double& out);
 ...
 * Forwarding
 */
ColorTemperatureIntfControllee::ColorTemperatureIntfControllee(BusAttachment& busAttachment, Ref<InterfaceControlleeModel> model, CdmBusObject& cdmBusObject, CdmControllee& cdmControllee)
    : m_impl(Impl::CreateInterface(busAttachment, dynamic_pointer_cast<ColorTemperatureIntfControlleeModel>(model), cdmBusObject, cdmControllee))
{}
...
{
    ColorTemperatureIntfControllee::Impl* interface = new ColorTemperatureIntfControllee::Impl(busAttachment, model, cdmBusObject, cdmControllee);
...
ColorTemperatureIntfControllee::Impl::Impl(BusAttachment& busAttachment, Ref<ColorTemperatureIntfControlleeModel> model, CdmBusObject& cdmBusObject, CdmControllee& cdmControllee) :
    InterfaceReceiver(cdmBusObject),
    m_busAttachment(busAttachment),
    m_cdmControllee(cdmControllee),
    m_methodHandlers(),
    m_ColorTemperatureModelInterface(model),
    InterfaceName(ColorTemperatureInterface::INTERFACE_NAME)
{
...
QStatus ColorTemperatureIntfControllee::Impl::OnGetProperty(const String& propName, MsgArg& msgarg)
{
    if (!(s_prop_Version.compare(propName))) {
        ...
        if (!(s_prop_Temperature.compare(propName))) {
            double value;
            auto status = m_ColorTemperatureModelInterface->GetTemperature(value);
            if (status != ER_OK) {
            ...
            double value;
            auto status = m_ColorTemperatureModelInterface->GetMinTemperature(value);
            if (status != ER_OK) {
                QCC_LogError(status, ("%s: failed to get actual property value from application. use previous value.", __func__));
                return ER_FAIL;
            }
            ...
            double value;
            auto status = m_ColorTemperatureModelInterface->GetMaxTemperature(value);
            if (status != ER_OK) {
                QCC_LogError(status, ("%s: failed to get actual property value from application. use previous value.", __func__));
                return ER_FAIL;
            }
...
QStatus ColorTemperatureIntfControllee::Impl::OnSetProperty(const String& propName, MsgArg& msgarg)
{
    if (!(s_prop_Temperature.compare(propName))) {
        ...
        if (clampTemperature(value, validValue) != ER_OK)
            return ER_INVALID_DATA;

        QStatus status = m_ColorTemperatureModelInterface->SetTemperature(validValue);
        if (status != ER_OK) {
...
void ColorTemperatureIntfControllee::Impl::OnMethodHandler(const InterfaceDescription::Member* member, Message& msg)
{
    bool isFound = false;
...
QStatus ColorTemperatureIntfControllee::Impl::clampTemperature(double value, double& out)
{

    double minValue;
    if (m_ColorTemperatureModelInterface->GetMinTemperature(minValue) != ER_OK)
        return ER_FAIL;

    double maxValue;
    if (m_ColorTemperatureModelInterface->GetMaxTemperature(maxValue) != ER_OK)
        return ER_FAIL;
