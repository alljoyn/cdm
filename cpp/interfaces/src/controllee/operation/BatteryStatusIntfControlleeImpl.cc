...
QStatus BatteryStatusIntfControllee::Impl::OnGetProperty(const String& propName, MsgArg& msgarg)
{
    ...
    if (!(s_prop_Version.compare(propName))) {
        ...
        if (!(s_prop_CurrentValue.compare(propName))) {
            uint8_t value;
            status = m_BatteryStatusModelInterface->GetCurrentValue(value);
            if (status != ER_OK) {
            ...
            bool value;
            status = m_BatteryStatusModelInterface->GetIsCharging(value);
            if (status != ER_OK) {
                QCC_LogError(status, ("%s: failed to get actual property value from application. use previous value.", __func__));
                return ER_FAIL;
            }
...
void BatteryStatusIntfControllee::Impl::OnMethodHandler(const InterfaceDescription::Member* member, Message& msg, CdmSideEffects& sideEffects)
{
