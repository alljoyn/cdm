...
QStatus SoilLevelIntfControllee::Impl::OnGetProperty(const String& propName, MsgArg& msgarg)
{
    ...
    if (!(s_prop_Version.compare(propName))) {
        ...
        if (!(s_prop_MaxLevel.compare(propName))) {
            uint8_t value;
            status = m_SoilLevelModelInterface->GetMaxLevel(value);
            if (status != ER_OK) {
            ...
            uint8_t value;
            status = m_SoilLevelModelInterface->GetTargetLevel(value);
            if (status != ER_OK) {
                QCC_LogError(status, ("%s: failed to get actual property value from application. use previous value.", __func__));
                return ER_FAIL;
            }
            ...
            status = m_SoilLevelModelInterface->GetSelectableLevels(value);
            if (status != ER_OK) {
                QCC_LogError(status, ("%s: failed to get actual property value from application. use previous value.", __func__));
                return ER_FAIL;
            }
...
void SoilLevelIntfControllee::Impl::OnMethodHandler(const InterfaceDescription::Member* member, Message& msg, CdmSideEffects& sideEffects)
{
