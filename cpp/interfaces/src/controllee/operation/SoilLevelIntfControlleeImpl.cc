    ...
    public SoilLevelInterface
{
  ...
  public:
    static Impl* CreateInterface(BusAttachment& busAttachment, Ref<SoilLevelIntfControlleeModel> model, CdmBusObject& cdmBusObject, CdmControllee& cdmControllee);
  ...
  private:
     ...
     */
    Impl(BusAttachment& busAttachment, Ref<SoilLevelIntfControlleeModel> model, CdmBusObject& cdmBusObject, CdmControllee& cdmControllee);
 ...
 * Forwarding
 */
SoilLevelIntfControllee::SoilLevelIntfControllee(BusAttachment& busAttachment, Ref<InterfaceControlleeModel> model, CdmBusObject& cdmBusObject, CdmControllee& cdmControllee)
    : m_impl(Impl::CreateInterface(busAttachment, dynamic_pointer_cast<SoilLevelIntfControlleeModel>(model), cdmBusObject, cdmControllee))
{}
...
{
    SoilLevelIntfControllee::Impl* interface = new SoilLevelIntfControllee::Impl(busAttachment, model, cdmBusObject, cdmControllee);
...
SoilLevelIntfControllee::Impl::Impl(BusAttachment& busAttachment, Ref<SoilLevelIntfControlleeModel> model, CdmBusObject& cdmBusObject, CdmControllee& cdmControllee) :
    InterfaceReceiver(cdmBusObject),
    m_busAttachment(busAttachment),
    m_cdmControllee(cdmControllee),
    m_methodHandlers(),
    m_SoilLevelModelInterface(model),
    InterfaceName(SoilLevelInterface::INTERFACE_NAME)
{
...
QStatus SoilLevelIntfControllee::Impl::OnSetProperty(const String& propName, MsgArg& msgarg)
{
    if (!(s_prop_MaxLevel.compare(propName))) {
        ...
        QStatus status = m_SoilLevelModelInterface->SetTargetLevel(validValue);
        if (status != ER_OK) {
...
void SoilLevelIntfControllee::Impl::OnMethodHandler(const InterfaceDescription::Member* member, Message& msg)
{
    bool isFound = false;
...
QStatus SoilLevelIntfControllee::Impl::ValidateTargetLevel(uint8_t value)
{

    std::vector<uint8_t> validValues;
    if (m_SoilLevelModelInterface->GetSelectableLevels(validValues) != ER_OK)
        return ER_FAIL;
...
QStatus SoilLevelIntfControllee::Impl::clampTargetLevel(uint8_t value, uint8_t& out)
{

    uint8_t minValue = 0;

    uint8_t maxValue;
    if (m_SoilLevelModelInterface->GetMaxLevel(maxValue) != ER_OK)
        return ER_FAIL;
