    ...
    public SpinSpeedLevelInterface
{
  ...
  public:
    static Impl* CreateInterface(BusAttachment& busAttachment, Ref<SpinSpeedLevelIntfControlleeModel> model, CdmBusObject& cdmBusObject);
  ...
  private:
     ...
     */
    Impl(BusAttachment& busAttachment, Ref<SpinSpeedLevelIntfControlleeModel> model, CdmBusObject& cdmBusObject);
 ...
 * Forwarding
 */
SpinSpeedLevelIntfControllee::SpinSpeedLevelIntfControllee(BusAttachment& busAttachment, Ref<InterfaceControlleeModel> model, CdmBusObject& cdmBusObject)
    : m_impl(Impl::CreateInterface(busAttachment, dynamic_pointer_cast<SpinSpeedLevelIntfControlleeModel>(model), cdmBusObject))
{}
...
{
    SpinSpeedLevelIntfControllee::Impl* interface = new SpinSpeedLevelIntfControllee::Impl(busAttachment, model, cdmBusObject);
...
SpinSpeedLevelIntfControllee::Impl::Impl(BusAttachment& busAttachment, Ref<SpinSpeedLevelIntfControlleeModel> model, CdmBusObject& cdmBusObject) :
...
QStatus SpinSpeedLevelIntfControllee::Impl::OnGetProperty(const String& propName, MsgArg& msgarg)
{
    ...
    if (!(s_prop_Version.compare(propName))) {
        ...
        if (!(s_prop_MaxLevel.compare(propName))) {
            uint8_t value;
            status = m_SpinSpeedLevelModelInterface->GetMaxLevel(value);
            if (status != ER_OK) {
            ...
            uint8_t value;
            status = m_SpinSpeedLevelModelInterface->GetTargetLevel(value);
            if (status != ER_OK) {
                QCC_LogError(status, ("%s: failed to get actual property value from application. use previous value.", __func__));
                return ER_FAIL;
            }
            ...
            status = m_SpinSpeedLevelModelInterface->GetSelectableLevels(value);
            if (status != ER_OK) {
                QCC_LogError(status, ("%s: failed to get actual property value from application. use previous value.", __func__));
                return ER_FAIL;
            }
...
void SpinSpeedLevelIntfControllee::Impl::OnMethodHandler(const InterfaceDescription::Member* member, Message& msg, CdmSideEffects& sideEffects)
{
