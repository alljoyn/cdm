    ...
    public LockedStatusInterface
{
  ...
  public:
    static Impl* CreateInterface(BusAttachment& busAttachment, Ref<LockedStatusIntfControlleeModel> model, CdmBusObject& cdmBusObject);
  ...
  private:
     ...
     */
    Impl(BusAttachment& busAttachment, Ref<LockedStatusIntfControlleeModel> model, CdmBusObject& cdmBusObject);
 ...
 * Forwarding
 */
LockedStatusIntfControllee::LockedStatusIntfControllee(BusAttachment& busAttachment, Ref<InterfaceControlleeModel> model, CdmBusObject& cdmBusObject)
    : m_impl(Impl::CreateInterface(busAttachment, dynamic_pointer_cast<LockedStatusIntfControlleeModel>(model), cdmBusObject))
{}
...
{
    LockedStatusIntfControllee::Impl* interface = new LockedStatusIntfControllee::Impl(busAttachment, model, cdmBusObject);
...
LockedStatusIntfControllee::Impl::Impl(BusAttachment& busAttachment, Ref<LockedStatusIntfControlleeModel> model, CdmBusObject& cdmBusObject) :
...
QStatus LockedStatusIntfControllee::Impl::OnGetProperty(const String& propName, MsgArg& msgarg)
{
    ...
    if (!(s_prop_Version.compare(propName))) {
        ...
        if (!(s_prop_IsLocked.compare(propName))) {
            bool value;
            status = m_LockedStatusModelInterface->GetIsLocked(value);
            if (status != ER_OK) {
...
void LockedStatusIntfControllee::Impl::OnMethodHandler(const InterfaceDescription::Member* member, Message& msg, CdmSideEffects& sideEffects)
{
