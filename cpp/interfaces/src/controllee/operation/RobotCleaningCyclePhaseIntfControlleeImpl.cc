    ...
    public RobotCleaningCyclePhaseInterface
{
  ...
  public:
    static Impl* CreateInterface(BusAttachment& busAttachment, Ref<RobotCleaningCyclePhaseIntfControlleeModel> model, CdmBusObject& cdmBusObject, CdmControllee& cdmControllee);
  ...
  private:
     ...
     */
    Impl(BusAttachment& busAttachment, Ref<RobotCleaningCyclePhaseIntfControlleeModel> model, CdmBusObject& cdmBusObject, CdmControllee& cdmControllee);
 ...
 * Forwarding
 */
RobotCleaningCyclePhaseIntfControllee::RobotCleaningCyclePhaseIntfControllee(BusAttachment& busAttachment, Ref<InterfaceControlleeModel> model, CdmBusObject& cdmBusObject, CdmControllee& cdmControllee)
    : m_impl(Impl::CreateInterface(busAttachment, dynamic_pointer_cast<RobotCleaningCyclePhaseIntfControlleeModel>(model), cdmBusObject, cdmControllee))
{}
...
{
    RobotCleaningCyclePhaseIntfControllee::Impl* interface = new RobotCleaningCyclePhaseIntfControllee::Impl(busAttachment, model, cdmBusObject, cdmControllee);
...
RobotCleaningCyclePhaseIntfControllee::Impl::Impl(BusAttachment& busAttachment, Ref<RobotCleaningCyclePhaseIntfControlleeModel> model, CdmBusObject& cdmBusObject, CdmControllee& cdmControllee) :
...
QStatus RobotCleaningCyclePhaseIntfControllee::Impl::OnGetProperty(const String& propName, MsgArg& msgarg)
{
    if (!(s_prop_Version.compare(propName))) {
        ...
        if (!(s_prop_CyclePhase.compare(propName))) {
            uint8_t value;
            auto status = m_RobotCleaningCyclePhaseModelInterface->GetCyclePhase(value);
            if (status != ER_OK) {
            ...
            auto status = m_RobotCleaningCyclePhaseModelInterface->GetSupportedCyclePhases(value);
            if (status != ER_OK) {
                QCC_LogError(status, ("%s: failed to get actual property value from application. use previous value.", __func__));
                return ER_FAIL;
            }
...
void RobotCleaningCyclePhaseIntfControllee::Impl::OnMethodHandler(const InterfaceDescription::Member* member, Message& msg)
{
    bool isFound = false;
...
void RobotCleaningCyclePhaseIntfControllee::Impl::OnGetVendorPhasesDescription(const InterfaceDescription::Member* member, Message& msg) {
    ...
    auto status = m_RobotCleaningCyclePhaseModelInterface->GetVendorPhasesDescription(arg_languageTag, arg_phasesDescription, errorCode, m_cdmControllee);

    // TODO this only allows one output arg
    MsgArg reply;
    CdmMsgCvt<std::vector<RobotCleaningCyclePhaseInterface::CyclePhaseDescriptor>> replyCvt;
