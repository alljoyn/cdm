...
class AudioVideoInputInterface : public CdmInterface {
  public:
    ...
    struct InputSource {
        ...
        inline bool operator==(const uint16_t key) {
            return key == id;
        }
    };
...
struct CdmMsgCvt<std::vector<AudioVideoInputInterface::SourceType>>
{
    ...
    {
        size_t l = value.size();
        std::vector<uint16_t> ints(l);

        for (size_t i = 0; i < l; ++i)
        {
            ints[i] = static_cast<uint16_t>(value[i]);
        }

        msgarg.Set("aq", ints.size(), ints.data());
        msgarg.Stabilize();
    }
...
struct CdmMsgCvt<AudioVideoInputInterface::InputSource>
{
    ...
    {
        uint16_t field_id{};
        uint16_t field_sourceType{};
        uint8_t field_signalPresence{};
        uint16_t field_portNumber{};
        const char* field_friendlyName{};
        msgarg.Get("(qqyqs)", &field_id, &field_sourceType, &field_signalPresence, &field_portNumber, &field_friendlyName);
        arg_.id = field_id;
    ...
    void set(MsgArg& msgarg, const AudioVideoInputInterface::InputSource arg_)
    {
        ...
        uint16_t field_sourceType = arg_.sourceType;
        uint8_t field_signalPresence = arg_.signalPresence;
        uint16_t field_portNumber = arg_.portNumber;
        const char* field_friendlyName = arg_.friendlyName.c_str();
        msgarg.Set("(qqyqs)", field_id, field_sourceType, field_signalPresence, field_portNumber, field_friendlyName);
    }
...
struct CdmMsgCvt<std::vector<AudioVideoInputInterface::InputSource>>
{
    void get(const MsgArg& msgarg, std::vector<AudioVideoInputInterface::InputSource>& arg_)
    {
        ...
        for (size_t i = 0; i < num; ++i)
        {
            ...
            uint16_t field_portNumber{};
            const char* field_friendlyName{};
            elems[i].Get("(qqyqs)", &field_id, &field_sourceType, &field_signalPresence, &field_portNumber, &field_friendlyName);
            arg_[i].id = field_id;
    ...
    void set(MsgArg& msgarg, const std::vector<AudioVideoInputInterface::InputSource>& arg_)
    {
        ...
        for (size_t i = 0; i < num; ++i)
        {
            ...
            uint16_t field_sourceType = arg_[i].sourceType;
            uint8_t field_signalPresence = arg_[i].signalPresence;
            uint16_t field_portNumber = arg_[i].portNumber;
            const char* field_friendlyName = arg_[i].friendlyName.c_str();
            elems[i].Set("(qqyqs)", field_id, field_sourceType, field_signalPresence, field_portNumber, field_friendlyName);
        }

        msgarg.Set("a(qqyqs)", num, elems.data());
        msgarg.Stabilize();
    }
