...
struct CdmMsgCvt<PlugInUnitsInterface::PlugInInfo>
{
    ...
    {
        msgarg.Get("(oub)", &value.objectPath, &value.deviceId, &value.pluggedIn);
    }

    void set(MsgArg& msgarg, PlugInUnitsInterface::PlugInInfo value)
    {
        msgarg.Set("(oub)", &value.objectPath, &value.deviceId, &value.pluggedIn);
    }
};
...
struct CdmMsgCvt<std::vector<PlugInUnitsInterface::PlugInInfo>>
{
    void get(const MsgArg& msgarg, std::vector<PlugInUnitsInterface::PlugInInfo>& value)
    {
        ...
        {
            elems[i].Get("(oub)", &value[i].objectPath, &value[i].deviceId, &value[i].pluggedIn);
        }
    }

    void set(MsgArg& msgarg, std::vector<PlugInUnitsInterface::PlugInInfo>& value)
    {
        ...
        for (size_t i = 0; i < num; ++i)
        {
            elems[i].Set("(oub)", &value[i].objectPath, &value[i].deviceId, &value[i].pluggedIn);
        }
