...
class AlertsIntfControlleeModel : public virtual InterfaceControlleeModel {
  public:
     ...
     */
    virtual QStatus GetAlerts(std::vector<AlertRecord>& out) const = 0;
     ...
     */
    virtual QStatus GetAlertCodesDescription(qcc::String arg_languageTag, std::vector<AlertCodesDescriptor>& arg_description, ErrorCode& error, CdmSideEffects& sideEffects) = 0;
    /**
     * Handler for method AcknowledgeAlert
     * @param[out] error Internal error code occurred during command execution
     * @return ER_OK on success
     */
    virtual QStatus AcknowledgeAlert(uint16_t arg_alertCode, ErrorCode& error, CdmSideEffects& sideEffects) = 0;
    /**
     * Handler for method AcknowledgeAllAlerts
     * @param[out] error Internal error code occurred during command execution
     * @return ER_OK on success
     */
    virtual QStatus AcknowledgeAllAlerts(ErrorCode& error, CdmSideEffects& sideEffects) = 0;
};
