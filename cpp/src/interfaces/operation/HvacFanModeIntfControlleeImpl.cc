...
HvacFanModeIntfControlleeImpl::~HvacFanModeIntfControlleeImpl()
{
}
...
QStatus HvacFanModeIntfControlleeImpl::OnGetProperty(const String& propName, MsgArg& val)
{
...
QStatus HvacFanModeIntfControlleeImpl::OnSetProperty(const String& propName, MsgArg& val)
{
    QStatus status = ER_OK;
    size_t i = 0;
...
void HvacFanModeIntfControlleeImpl::OnMethodHandler(const InterfaceDescription::Member* member, Message& msg)
{
    QStatus status = ER_OK;
    bool isFound = false;
...
QStatus HvacFanModeIntfControlleeImpl::SetMode(const uint16_t mode)
{
    size_t i = 0;
...
QStatus HvacFanModeIntfControlleeImpl::SetSupportedModes(const SupportedModes& modes)
{
    QStatus status = ER_OK;
    uint32_t i=0;
    bool diff = false;
    ...
    for (i = 0; i < modes.size(); i++) {
        if (modes[i] < MIN_HVAC_FAN_MODE || modes[i] > MAX_HVAC_FAN_MODE) {
            status = ER_INVALID_DATA;
