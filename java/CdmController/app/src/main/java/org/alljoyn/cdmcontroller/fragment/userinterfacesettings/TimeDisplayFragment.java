...
public class TimeDisplayFragment extends InterfaceFragment {
    private static final String INTERFACE_NAME = "org.alljoyn.SmartSpaces.UserInterfaceSettings.TimeDisplay";

    @Override
    protected void generatePropertyView(CustomView properties, CustomView methods) {
        View displayTimeFormatView = new ReadWriteValuePropertyView(getActivity(), this.device, this.objPath, INTERFACE_NAME, "DisplayTimeFormat", null);
        properties.addView(displayTimeFormatView);

        View supportedDisplayTimeFormatsView = new ReadOnlyValuePropertyView(getActivity(), this.device, this.objPath, INTERFACE_NAME, "SupportedDisplayTimeFormats", null);
        properties.addView(supportedDisplayTimeFormatsView);
