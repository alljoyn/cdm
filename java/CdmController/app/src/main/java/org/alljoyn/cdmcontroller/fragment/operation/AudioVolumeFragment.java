...
public class AudioVolumeFragment extends InterfaceFragment {
    private static final String INTERFACE_NAME = "org.alljoyn.SmartSpaces.Operation.AudioVolume";

    @Override
    protected void generatePropertyView(CustomView properties, CustomView methods) {
        View volumeView = new ReadWriteValuePropertyView(getActivity(), this.device, this.objPath, INTERFACE_NAME, "Volume", null);
        properties.addView(volumeView);

        View maxVolumeView = new ReadOnlyValuePropertyView(getActivity(), this.device, this.objPath, INTERFACE_NAME, "MaxVolume", null);
        properties.addView(maxVolumeView);

        View muteView = new ReadWriteValuePropertyView(getActivity(), this.device, this.objPath, INTERFACE_NAME, "Mute", null);
