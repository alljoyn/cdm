...
public class TemperatureDisplayFragment extends InterfaceFragment {
    private static final String INTERFACE_NAME = "org.alljoyn.SmartSpaces.UserInterfaceSettings.TemperatureDisplay";

    @Override
    protected void generatePropertyView(CustomView properties, CustomView methods) {
        View displayTemperatureUnitView = new ReadWriteValuePropertyView(getActivity(), this.device, this.objPath, INTERFACE_NAME, "DisplayTemperatureUnit", null);
        properties.addView(displayTemperatureUnitView);

        View supportedDisplayTemperatureUnitsView = new ReadOnlyValuePropertyView(getActivity(), this.device, this.objPath, INTERFACE_NAME, "SupportedDisplayTemperatureUnits", null);
        properties.addView(supportedDisplayTemperatureUnitsView);
