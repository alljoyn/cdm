...
import org.alljoyn.smartspaces.operation.Color;
...
public class ColorFragment extends InterfaceFragment {
    private static final String INTERFACE_NAME = "org.alljoyn.SmartSpaces.Operation.Color";

    @Override
    protected void generatePropertyView(CustomView properties, CustomView methods) {
        View hueView = new ReadWriteValuePropertyView(getActivity(), this.device, this.objPath, INTERFACE_NAME, "Hue", null);
        properties.addView(hueView);

        View saturationView = new ReadWriteValuePropertyView(getActivity(), this.device, this.objPath, INTERFACE_NAME, "Saturation", null);
        properties.addView(saturationView);
