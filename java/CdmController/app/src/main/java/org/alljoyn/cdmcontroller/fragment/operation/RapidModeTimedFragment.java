...
import org.alljoyn.smartspaces.operation.RapidModeTimed;
...
public class RapidModeTimedFragment extends InterfaceFragment {
    private static final String INTERFACE_NAME = "org.alljoyn.SmartSpaces.Operation.RapidModeTimed";

    @Override
    protected void generatePropertyView(CustomView properties, CustomView methods) {
        View rapidModeMinutesRemainingView = new ReadWriteValuePropertyView(getActivity(), this.device, this.objPath, INTERFACE_NAME, "RapidModeMinutesRemaining", null);
        properties.addView(rapidModeMinutesRemainingView);

        View maxSetMinutesView = new ReadOnlyValuePropertyView(getActivity(), this.device, this.objPath, INTERFACE_NAME, "MaxSetMinutes", null);
        properties.addView(maxSetMinutesView);
