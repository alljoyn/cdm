...
import org.alljoyn.smartspaces.operation.SpinSpeedLevel;
...
public class SpinSpeedLevelFragment extends InterfaceFragment {
    private static final String INTERFACE_NAME = "org.alljoyn.SmartSpaces.Operation.SpinSpeedLevel";

    @Override
    protected void generatePropertyView(CustomView properties, CustomView methods) {
        View maxLevelView = new ReadOnlyValuePropertyView(getActivity(), this.device, this.objPath, INTERFACE_NAME, "MaxLevel", null);
        properties.addView(maxLevelView);

        View targetLevelView = new ReadWriteValuePropertyView(getActivity(), this.device, this.objPath, INTERFACE_NAME, "TargetLevel", null);
        properties.addView(targetLevelView);

        View selectableLevelsView = new ReadOnlyValuePropertyView(getActivity(), this.device, this.objPath, INTERFACE_NAME, "SelectableLevels", null);
        properties.addView(selectableLevelsView);
