...
import org.alljoyn.smartspaces.operation.HvacFanMode;
...
public class HvacFanModeFragment extends InterfaceFragment {
    private static final String INTERFACE_NAME = "org.alljoyn.SmartSpaces.Operation.HvacFanMode";

    @Override
    protected void generatePropertyView(CustomView properties, CustomView methods) {
        View modeView = new ReadWriteValuePropertyView(getActivity(), this.device, this.objPath, INTERFACE_NAME, "Mode", null);
        properties.addView(modeView);

        View supportedModesView = new ReadOnlyValuePropertyView(getActivity(), this.device, this.objPath, INTERFACE_NAME, "SupportedModes", null);
        properties.addView(supportedModesView);
