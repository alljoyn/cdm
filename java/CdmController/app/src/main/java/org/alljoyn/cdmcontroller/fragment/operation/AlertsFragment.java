...
import org.alljoyn.smartspaces.operation.Alerts;
...
public class AlertsFragment extends InterfaceFragment {
    private static final String INTERFACE_NAME = "org.alljoyn.SmartSpaces.Operation.Alerts";

    @Override
    protected void generatePropertyView(CustomView properties, CustomView methods) {
        View alertsView = new ReadOnlyValuePropertyView(getActivity(), this.device, this.objPath, INTERFACE_NAME, "Alerts", null);
        properties.addView(alertsView);

        View getAlertCodesDescriptionView = new MethodView(getActivity(), this.device, this.objPath, INTERFACE_NAME, "GetAlertCodesDescription", "languageTag");
        methods.addView(getAlertCodesDescriptionView);

        View acknowledgeAlertView = new MethodView(getActivity(), this.device, this.objPath, INTERFACE_NAME, "AcknowledgeAlert", "alertCode");
        methods.addView(acknowledgeAlertView);

        View acknowledgeAllAlertsView = new MethodView(getActivity(), this.device, this.objPath, INTERFACE_NAME, "AcknowledgeAllAlerts");
        methods.addView(acknowledgeAllAlertsView);
