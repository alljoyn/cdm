...
import org.alljoyn.smartspaces.operation.RobotCleaningCyclePhase;
...
public class RobotCleaningCyclePhaseFragment extends InterfaceFragment {
    private static final String INTERFACE_NAME = "org.alljoyn.SmartSpaces.Operation.RobotCleaningCyclePhase";

    @Override
    protected void generatePropertyView(CustomView properties, CustomView methods) {
        View cyclePhaseView = new ReadOnlyValuePropertyView(getActivity(), this.device, this.objPath, INTERFACE_NAME, "CyclePhase", null);
        properties.addView(cyclePhaseView);

        View supportedCyclePhasesView = new ReadOnlyValuePropertyView(getActivity(), this.device, this.objPath, INTERFACE_NAME, "SupportedCyclePhases", null);
        properties.addView(supportedCyclePhasesView);

        View getVendorPhasesDescriptionView = new MethodView(getActivity(), this.device, this.objPath, INTERFACE_NAME, "GetVendorPhasesDescription", "languageTag");
