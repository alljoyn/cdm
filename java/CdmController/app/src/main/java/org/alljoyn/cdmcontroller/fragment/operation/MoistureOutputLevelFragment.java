...
import org.alljoyn.smartspaces.operation.MoistureOutputLevel;
...
public class MoistureOutputLevelFragment extends InterfaceFragment {
    private static final String INTERFACE_NAME = "org.alljoyn.SmartSpaces.Operation.MoistureOutputLevel";

    @Override
    protected void generatePropertyView(CustomView properties, CustomView methods) {
        View moistureOutputLevelView = new ReadWriteValuePropertyView(getActivity(), this.device, this.objPath, INTERFACE_NAME, "MoistureOutputLevel", null);
        properties.addView(moistureOutputLevelView);

        View maxMoistureOutputLevelView = new ReadOnlyValuePropertyView(getActivity(), this.device, this.objPath, INTERFACE_NAME, "MaxMoistureOutputLevel", null);
        properties.addView(maxMoistureOutputLevelView);

        View autoModeView = new ReadWriteValuePropertyView(getActivity(), this.device, this.objPath, INTERFACE_NAME, "AutoMode", null);
