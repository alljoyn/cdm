...
public class AudioVideoInputFragment extends InterfaceFragment {
    private static final String INTERFACE_NAME = "org.alljoyn.SmartSpaces.Operation.AudioVideoInput";

    @Override
    protected void generatePropertyView(CustomView properties, CustomView methods) {
        View inputSourceIdView = new ReadWriteValuePropertyView(getActivity(), this.device, this.objPath, INTERFACE_NAME, "InputSourceId", null);
        properties.addView(inputSourceIdView);

        View supportedInputSourcesView = new ReadOnlyValuePropertyView(getActivity(), this.device, this.objPath, INTERFACE_NAME, "SupportedInputSources", null);
        properties.addView(supportedInputSourcesView);
