...
import org.alljoyn.smartspaces.environment.WindDirection;
...
public class WindDirectionFragment extends InterfaceFragment {
    private static final String INTERFACE_NAME = "org.alljoyn.SmartSpaces.Environment.WindDirection";

    @Override
    protected void generatePropertyView(CustomView properties, CustomView methods) {
        View horizontalDirectionView = new ReadWriteValuePropertyView(getActivity(), this.device, this.objPath, INTERFACE_NAME, "HorizontalDirection", null);
        properties.addView(horizontalDirectionView);

        View horizontalMaxView = new ReadOnlyValuePropertyView(getActivity(), this.device, this.objPath, INTERFACE_NAME, "HorizontalMax", null);
        properties.addView(horizontalMaxView);

        View horizontalAutoModeView = new ReadWriteBoolPropertyView(getActivity(), this.device, this.objPath, INTERFACE_NAME, "HorizontalAutoMode");
        properties.addView(horizontalAutoModeView);

        View verticalDirectionView = new ReadWriteValuePropertyView(getActivity(), this.device, this.objPath, INTERFACE_NAME, "VerticalDirection", null);
        properties.addView(verticalDirectionView);

        View verticalMaxView = new ReadOnlyValuePropertyView(getActivity(), this.device, this.objPath, INTERFACE_NAME, "VerticalMax", null);
        properties.addView(verticalMaxView);

        View verticalAutoModeView = new ReadWriteBoolPropertyView(getActivity(), this.device, this.objPath, INTERFACE_NAME, "VerticalAutoMode");
