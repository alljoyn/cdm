...
public class CurrentAirQualityLevelFragment extends InterfaceFragment {
    private static final String INTERFACE_NAME = "org.alljoyn.SmartSpaces.Environment.CurrentAirQualityLevel";

    @Override
    protected void generatePropertyView(CustomView properties, CustomView methods) {
        View contaminantTypeView = new ReadOnlyValuePropertyView(getActivity(), this.device, this.objPath, INTERFACE_NAME, "ContaminantType", null);
        properties.addView(contaminantTypeView);

        View currentLevelView = new ReadOnlyValuePropertyView(getActivity(), this.device, this.objPath, INTERFACE_NAME, "CurrentLevel", null);
