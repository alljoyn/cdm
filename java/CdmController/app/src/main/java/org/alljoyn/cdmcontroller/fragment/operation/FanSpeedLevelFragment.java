...
import org.alljoyn.smartspaces.operation.FanSpeedLevel;
...
public class FanSpeedLevelFragment extends InterfaceFragment {
    private static final String INTERFACE_NAME = "org.alljoyn.SmartSpaces.Operation.FanSpeedLevel";

    @Override
    protected void generatePropertyView(CustomView properties, CustomView methods) {
        View fanSpeedLevelView = new ReadWriteValuePropertyView(getActivity(), this.device, this.objPath, INTERFACE_NAME, "FanSpeedLevel", null);
        properties.addView(fanSpeedLevelView);

        View maxFanSpeedLevelView = new ReadOnlyValuePropertyView(getActivity(), this.device, this.objPath, INTERFACE_NAME, "MaxFanSpeedLevel", null);
        properties.addView(maxFanSpeedLevelView);
