...
#import "alljoyn/cdm/interfaces/operation/RobotCleaningCyclePhaseIntfController.h"
...
@interface RobotCleaningCyclePhaseViewController() <UIPickerViewDelegate, UIPickerViewDataSource>
@property ajn::services::CdmController *cdmController;
@property (nonatomic, strong) Device* device;
@property RobotCleaningCyclePhaseListener *listener;
@property std::shared_ptr<ajn::services::RobotCleaningCyclePhaseIntfController> robotCleaningCyclePhaseIntfController;
...
@implementation RobotCleaningCyclePhaseViewController

- (instancetype)initWithDevice:(Device *)device andController:(ajn::services::CdmController *)cdmController
{
    self = [super init];
    if (self) {
        self.title = @"robot cleaning cycle phase";
        _cdmController = cdmController;
        _device = device;

        _listener = new RobotCleaningCyclePhaseListener(self);

        _cdmInterface = _cdmController->CreateInterface(ajn::services::ROBOT_CLEANING_CYCLE_PHASE_INTERFACE,
        ...
        _robotCleaningCyclePhaseIntfController = std::static_pointer_cast<ajn::services::RobotCleaningCyclePhaseIntfController>(_cdmInterface);
...
-(UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    if (indexPath.section == SECTION_PROPERTY) {
        if(indexPath.row == 0) {
            ...
            status = _robotCleaningCyclePhaseIntfController->GetSupportedCyclePhases();
            if(status != ER_OK) {
                NSLog(@"Failed to get GetSupportedCyclePhases");
            }                

            status = _robotCleaningCyclePhaseIntfController->GetCyclePhase();
            if(status != ER_OK) {
    ...
    if([methodName isEqualToString:@"GetVendorPhasesDescription"]){
        ...
        [alertController addAction:[UIAlertAction actionWithTitle:@"Run"
                                                          ...
                                                          handler:^(UIAlertAction * _Nonnull action) {
                                                              ...
                                                              _robotCleaningCyclePhaseIntfController->GetVendorPhasesDescription(languageTagCommand);
                                                          }]];
...
- (void)pickerView:(UIPickerView *)pickerView didSelectRow:(NSInteger)row inComponent:(NSInteger)component
{
   switch(pickerView.tag) {
        ...
        {
//            uint8_t cyclePhase = self.selectorValuesForCyclePhase->at(row);
//            _robotCleaningCyclePhaseIntfController->SetCyclePhase(cyclePhase);
