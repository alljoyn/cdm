...
#import "alljoyn/cdm/interfaces/operation/DishWashingCyclePhaseIntfController.h"
...
@interface DishWashingCyclePhaseViewController() <UIPickerViewDelegate, UIPickerViewDataSource>
@property ajn::services::CdmController *cdmController;
@property (nonatomic, strong) Device* device;
@property DishWashingCyclePhaseListener *listener;
@property std::shared_ptr<ajn::services::DishWashingCyclePhaseIntfController> dishWashingCyclePhaseIntfController;
...
@implementation DishWashingCyclePhaseViewController

- (instancetype)initWithDevice:(Device *)device andController:(ajn::services::CdmController *)cdmController
{
    self = [super init];
    if (self) {
        self.title = @"dish washing cycle phase";
        _cdmController = cdmController;
        _device = device;

        _listener = new DishWashingCyclePhaseListener(self);

        _cdmInterface = _cdmController->CreateInterface(ajn::services::DISH_WASHING_CYCLE_PHASE_INTERFACE,
        ...
        _dishWashingCyclePhaseIntfController = std::static_pointer_cast<ajn::services::DishWashingCyclePhaseIntfController>(_cdmInterface);
...
-(UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    if (indexPath.section == SECTION_PROPERTY) {
        if(indexPath.row == 0) {
            ...
            status = _dishWashingCyclePhaseIntfController->GetSupportedCyclePhases();
            if(status != ER_OK) {
                NSLog(@"Failed to get GetSupportedCyclePhases");
            }                

            status = _dishWashingCyclePhaseIntfController->GetCyclePhase();
            if(status != ER_OK) {
    ...
    if([methodName isEqualToString:@"GetVendorPhasesDescription"]){
        ...
        [alertController addAction:[UIAlertAction actionWithTitle:@"Run"
                                                          ...
                                                          handler:^(UIAlertAction * _Nonnull action) {
                                                              ...
                                                              _dishWashingCyclePhaseIntfController->GetVendorPhasesDescription(languageTagCommand);
                                                          }]];
...
- (void)pickerView:(UIPickerView *)pickerView didSelectRow:(NSInteger)row inComponent:(NSInteger)component
{
   switch(pickerView.tag) {
        ...
        {
//            uint8_t cyclePhase = self.selectorValuesForCyclePhase->at(row);
//            _dishWashingCyclePhaseIntfController->SetCyclePhase(cyclePhase);
