...
ResourceSavingListener::ResourceSavingListener(ResourceSavingViewController *viewController):viewController(viewController)
{
...
void ResourceSavingListener::UpdateResourceSavingMode(const bool value)
{
    NSString *valueAsStr = [NSString stringWithFormat:@"%@", [CDMUtil boolToNSString:value]];
    NSLog(@"Got ResourceSavingMode: %@", valueAsStr);
    dispatch_async(dispatch_get_main_queue(), ^{
        viewController.resourceSavingModeCell.value.text = valueAsStr;
    });
}
...
{
    UpdateResourceSavingMode(value);
}
...
{
    if(status == ER_OK){
        NSLog(@"SetResourceSavingMode: succeeded");
    }else {
        NSLog(@"SetResourceSavingMode: failed");
    }
}
