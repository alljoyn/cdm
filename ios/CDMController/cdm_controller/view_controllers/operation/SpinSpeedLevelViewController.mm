...
#import "interfaces/controller/operation/SpinSpeedLevelIntfController.h"

static NSInteger NUM_MEMBER_CATEGORIES = 1;
static NSInteger NUM_PROPERTIES = 3;
static NSInteger NUM_METHODS = 0;

@interface SpinSpeedLevelViewController() 
@property ajn::services::CdmController *cdmController;
@property (nonatomic, strong) Device* device;
@property std::shared_ptr<SpinSpeedLevelListener> listener;
...
- (instancetype)initWithDevice:(Device *)device andController:(ajn::services::CdmController *)cdmController
{
    ...
    if (self) {
        ...
        _listener = std::shared_ptr<SpinSpeedLevelListener>(new SpinSpeedLevelListener(self));

        _cdmInterface = _cdmController->CreateInterface(ajn::services::CdmInterface::GetInterfaceName(ajn::services::SPIN_SPEED_LEVEL_INTERFACE),
...
-(void) updateValue:(NSString *)value forProperty:(NSString *)property
{
    if([property isEqualToString:@"TargetLevel"]){
        _spinSpeedLevelIntfController->SetTargetLevel((uint8_t)[value longLongValue]);
    }
}
