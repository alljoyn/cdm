...
#import "interfaces/controller/userinterfacesettings/LanguageDisplayIntfController.h"

static NSInteger NUM_MEMBER_CATEGORIES = 1;
static NSInteger NUM_PROPERTIES = 2;
static NSInteger NUM_METHODS = 0;

@interface LanguageDisplayViewController() 
@property ajn::services::CdmController *cdmController;
@property (nonatomic, strong) Device* device;
@property std::shared_ptr<LanguageDisplayListener> listener;
...
- (instancetype)initWithDevice:(Device *)device andController:(ajn::services::CdmController *)cdmController
{
    ...
    if (self) {
        ...
        _listener = std::shared_ptr<LanguageDisplayListener>(new LanguageDisplayListener(self));

        _cdmInterface = _cdmController->CreateInterface(ajn::services::CdmInterface::GetInterfaceName(ajn::services::LANGUAGE_DISPLAY_INTERFACE),
...
-(UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    if (indexPath.section == SECTION_PROPERTY) {
        if(indexPath.row == 0) {
            _displayLanguageCell  = [tableView dequeueReusableCellWithIdentifier:READ_WRITE_TABLE_VIEW_CELL];
            _displayLanguageCell.label.text = @"DisplayLanguage";
            _displayLanguageCell.delegate = self;
        ...
        if(indexPath.row == 1) {
            _supportedDisplayLanguagesCell  = [tableView dequeueReusableCellWithIdentifier:READ_ONLY_TABLE_VIEW_CELL];
            _supportedDisplayLanguagesCell.label.text = @"SupportedDisplayLanguages";
            ...
            return _supportedDisplayLanguagesCell;
        }
    }     
    return nil;
}
...
{
    if([property isEqualToString:@"DisplayLanguage"]){
        _languageDisplayIntfController->SetDisplayLanguage(qcc::String([value cStringUsingEncoding:NSUTF8StringEncoding]));
    }
}
