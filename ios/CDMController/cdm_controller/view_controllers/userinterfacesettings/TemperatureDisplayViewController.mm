...
#import "interfaces/controller/userinterfacesettings/TemperatureDisplayIntfController.h"

static NSInteger NUM_MEMBER_CATEGORIES = 1;
static NSInteger NUM_PROPERTIES = 2;
static NSInteger NUM_METHODS = 0;

@interface TemperatureDisplayViewController() 
@property ajn::services::CdmController *cdmController;
@property (nonatomic, strong) Device* device;
@property std::shared_ptr<TemperatureDisplayListener> listener;
...
- (instancetype)initWithDevice:(Device *)device andController:(ajn::services::CdmController *)cdmController
{
    ...
    if (self) {
        ...
        _listener = std::shared_ptr<TemperatureDisplayListener>(new TemperatureDisplayListener(self));

        _cdmInterface = _cdmController->CreateInterface(ajn::services::CdmInterface::GetInterfaceName(ajn::services::TEMPERATURE_DISPLAY_INTERFACE),
...
-(UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    if (indexPath.section == SECTION_PROPERTY) {
        if(indexPath.row == 0) {
            _displayTemperatureUnitCell  = [tableView dequeueReusableCellWithIdentifier:READ_WRITE_TABLE_VIEW_CELL];
            _displayTemperatureUnitCell.label.text = @"DisplayTemperatureUnit";
            _displayTemperatureUnitCell.delegate = self;
        ...
        if(indexPath.row == 1) {
            _supportedDisplayTemperatureUnitsCell  = [tableView dequeueReusableCellWithIdentifier:READ_ONLY_TABLE_VIEW_CELL];
            _supportedDisplayTemperatureUnitsCell.label.text = @"SupportedDisplayTemperatureUnits";
            ...
            return _supportedDisplayTemperatureUnitsCell;
        }
    }     
    return nil;
}
...
{
    if([property isEqualToString:@"DisplayTemperatureUnit"]){
        _temperatureDisplayIntfController->SetDisplayTemperatureUnit((uint8_t)[value longLongValue]);
    }
}
