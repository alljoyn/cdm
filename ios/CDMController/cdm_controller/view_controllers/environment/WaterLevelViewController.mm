...
- (instancetype)initWithDevice:(Device *)device andController:(ajn::services::CdmController *)cdmController
{
    ...
    if (self) {
        ...
        _waterLevelIntfController = std::static_pointer_cast<ajn::services::WaterLevelIntfController>(_cdmInterface);
...
-(UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    if (indexPath.section == SECTION_PROPERTY) {
        if(indexPath.row == 0) {
            _supplySourceCell  = [tableView dequeueReusableCellWithIdentifier:READ_ONLY_TABLE_VIEW_CELL];
            _supplySourceCell.label.text = @"SupplySource";

            status = _waterLevelIntfController->GetSupplySource();
            if(status != ER_OK) {
                NSLog(@"Failed to get GetSupplySource");
            }

            return _supplySourceCell;
        }
        if(indexPath.row == 1) {
            ...
            status = _waterLevelIntfController->GetCurrentLevel();
            if(status != ER_OK) {
        ...
        if(indexPath.row == 2) {
            ...
            status = _waterLevelIntfController->GetMaxLevel();
            if(status != ER_OK) {
